# sample input

# arr=[100,200,300,400,500,600,700,200]
#    0    1   2   3   4   5   6   7
#                 L        R
# output
# odd_position_elements[200,400,600,800]
# reverse=[800,600,400,200]
# insert these elements into odd_position
# result=[100,800,300,600,500,400,700,200]


# sample input2
# arr=[10,20,30,40,50,60]
#     0  1  2  3  4  5
# odd_position_elements=[20,40,60]
# reverse_odd_position_elements=[60,40,20]
# result=[10,60,30,40,10]

arr=[100,800,300,600,500,600,700,200]
#    0  1     2    3  4   5  6   7
#    i
odd_position_elements=list() #or []

for i in range(0,len(arr)):
    if i%2!=0:
        odd_position_elements.append(arr[i])
print(odd_position_elements)

odd_position_elements.reverse()

for i in range(1,len(arr),2):#i==1,3,5,7,9
    element=odd_position_elements.pop()#200
    arr[i]=element#arr[i]=element
    print(arr)
